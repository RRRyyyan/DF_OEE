...
namespace PMCDash.Controllers
{
    ...
    public class ScheduleController : BaseApiController
    {
        ...
        public ActionResponse<List<ScheduleInformation>> GetScheduleInformationsByFactory([FromBody] RequestFactory request)
        {
            ...
            //var wipSatutsText = new string[] { "收料確認", "派工完成", "加工開始", "加工完成", "暫停加工" };
            //var delayDays = new int[] { 10, 20, 30, 0 };
            //for (int i = 0; i < 70; i++)
            //{
            //    var temp = new OrderInformation(orderNo: $@"104111100{i,2:00}", oPNo: 66, opName: "攻牙",
            //        productNo: $@"1111{i,4:0000}", requireCount: 3000, currentCount: 1500, dueDate: "2021-12-03");
            //    result.Add(new ScheduleInformation(temp, wipStatus[i % 5], DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
            //        DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), delayDays[i % 4]));
                        ...
                        if (SqlData.HasRows)
                        {
                            while (SqlData.Read())
                            {
                                result.Add(new DelayOrderList
                                {
                                    ...
                                    AssignDate = SqlData["AssignDate"].ToString().Trim()
                                });
                            }
                        }
                    }
